...
class DownloadLogger:
    ...
    def _get_base_message(self):
        if len(str(self._downloadIndex)) != len(str(self._downloadTotal)):
            zero_to_pad = len(str(self._downloadTotal)) - \
                len(str(self._downloadIndex))
...
class DeemixDownloader:
    ...
    def download_songs(self, songs: List[ProcessedSong]):
        ...
        download_objs = {v.spotify_id: {'song': v, 'download_obj': generateDownloadObject(
            self.dz, v.deezer_url, self.config['maxBitrate'])} for v in self.songs_to_download}
        threads = get_threads()
    ...
    def update_download_report(self, download_object, requested_song: ProcessedSong, listener: LogListener, dl_logger):
        ...
        if download_object.downloaded == 1:
            ...
            if download_skipped:
                dl_logger.info(action='FINSIHED',
                               message=f'Skipping, already downloaded')
                status = True
                md5 = get_md5(f)
                ...
                dl_logger.warn(
                    action='FINSIHED', message=f'Failed, downloaded but could not find {f}')
                errors = [f"Downloaded but could not find {f}"]

        elif download_object.failed == 1:
            dl_logger.warn(
                action='FINSIHED', message=f'Failed, download for DeezerId: {download_object.single["trackAPI"]["id"]} error: {download_object.errors[0]["message"]}')
...
def get_threads():
    ...
    try:
        ...
        threads = threads if int(
            config["THREADS"]) <= machine_threads else machine_threads
    except:
        threads = machine_threads
        logger.warning(
            f'THREADING - Failed parsing int from "{config["THREADS"]}", using max available ({str(machine_threads)})')
...
def check_deemix_config():
    ...
    if not os.path.isdir(config["DEEMIX_DOWNLOAD_PATH"]):
        logger.warning(
            f'{config["DEEMIX_DOWNLOAD_PATH"]} does not exist, creating')
