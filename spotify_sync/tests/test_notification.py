...
from spotify_sync.notification import (
    NotificationType,
    NotificationClient,
    PushoverClient,
)
...
class TestNotificationStrategy(unittest.TestCase):
    ...
    def test_should_send_info(self):
        client = TestNotificationClient(
            Config(None, "test", {"NOTIFICATION_STRATEGY": 1})
        )
    ...
    def test_should_send_change(self):
        client = TestNotificationClient(
            Config(None, "test", {"NOTIFICATION_STRATEGY": 2})
        )
    ...
    def test_should_send_failure(self):
        client = TestNotificationClient(
            Config(None, "test", {"NOTIFICATION_STRATEGY": 3})
        )
    ...
    def test_send_info(self):
        ...
        self.mocker.patch.object(
            client, "send_message", return_value=unittest.mock.MagicMock()
        )
    ...
    def test_send_change(self):
        ...
        client.send_message.assert_called_once()
        client.send_message.assert_called_with(
            "test message", NotificationType.CHANGE
        )
    ...
    def test_send_failure(self):
        ...
        client.send_message.assert_called_once()
        client.send_message.assert_called_with(
            "test message", NotificationType.FAILURE
        )
...
def pushover_client(mocker, pushover_config):
    ...
    mock_user.return_value.send_message.return_value = (
